...
city_prop_df = 
  prop.test(x = baltimore_df %>% pull(as.numeric(baltimore_df[2])), n = baltimore_df %>% pull(as.numeric(baltimore_df[1]), p = NULL, alternative = c("two.sided"), conf.level = 0.95, correct = TRUE)
...
prop_test = function(city_state) {
  ...
  city_prop_df = 
    city_choice_df %>% 
  prop.test(x = totalhomicides, n = totalhomicides)%>% 
  broom::tidy()
}
...
p_value_baltimore = prop_test(baltimore_df) 
